...
export class WeatherWidget extends Widget {
  constructor(widgetId, data, uniqueWidgetData) {
    ...
    this.weatherIconsDict = {
      0: ["clear-night.svg", "clear-day.svg"],
    ...
    if (
      this.uniqueWidgetData.longitude == null &&
      this.uniqueWidgetData.latitude == null
    ) {
  ...
  getWeatherData() {
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        this.setWeatherIcon(
          1,
          this.weatherIconsDict[data["current_weather"]["weathercode"]][
            data["current_weather"]["is_day"]
          ],
          data["current_weather"]["temperature"],
          this.uniqueWidgetData.loc
        );
      })
      .catch((error) => {
        console.error(error);
  ...
  setWeatherIcon(day, weatherPath, temp) {
    const dayPath = this.widgetPath
      .querySelector(".weatherWindow")
  ...
  submitLocationListener() {
    this.widgetPath
      ...
      .querySelector("#submitWeatherLoc")
      .addEventListener("click", (event) => {
        if (
          this.widgetPath
            .querySelector(".location-popup")
            .querySelector("#latitudeField").value !== "" &&
          this.widgetPath
            .querySelector(".location-popup")
            .querySelector("#longitudeField").value !== ""
        ) {
          this.uniqueWidgetData.latitude = this.widgetPath
            .querySelector(".location-popup")
            .querySelector("#latitudeField").value;
          this.uniqueWidgetData.longitude = this.widgetPath
            .querySelector(".location-popup")
            .querySelector("#longitudeField").value;
          ...
          this.widgetPath.querySelector(".location-popup").style.display =
            "none";
          this.widgetPath.querySelector(".weatherWindow").style.display =
            "block";

          const url = `https://geocode.maps.co/reverse?lat=${this.uniqueWidgetData.latitude}&lon=${this.uniqueWidgetData.longitude}`;
          fetch(url)
            .then((response) => {
              if (response.status === 200) {
                return response.json();
            ...
            .then((data) => {
              ...
              if (data.address.hasOwnProperty("village")) {
                this.uniqueWidgetData.loc = data.address.village;
              } else {
                this.uniqueWidgetData.loc = data.address.city;
              }
              this.getWeatherData()
              this.setWeatherIcon(
                1,
